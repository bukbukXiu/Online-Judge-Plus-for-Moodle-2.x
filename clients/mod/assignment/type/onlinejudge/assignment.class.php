...
class assignment_onlinejudge extends assignment_upload {
    ...
    function setup_elements(&$mform ) {
        ...
        // Много и сразу ============================================================================================
        $mform->addElement('select', '10atonce', get_string('10 at once?'), $ynoptions);
        $mform->setAdvanced('10atonce');
    ...
    function extend_settings_navigation($assignmentnode) {
        ...
        //ДОБАВЛЕННЫЕ:
        if (has_capability('mod/assignment:addinstance', $PAGE->cm->context)) {
			$string = get_string('generateojassignments','assignment_onlinejudge');
			$link = $CFG->wwwroot.'/mod/assignment/type/onlinejudge/generatetestcases.php?courseid='.$this->cm->course;
			$assignmentnode->add($string, $link, navigation_node::TYPE_SETTING);
		}
		if (has_capability('mod/assignment:addinstance', $PAGE->cm->context)) {
			$string = get_string('replaceassignment','assignment_onlinejudge');
			$link = $CFG->wwwroot.'/mod/assignment/type/onlinejudge/generateassignments.php?id='.$this->cm->id;
			$assignmentnode->add($string, $link, navigation_node::TYPE_SETTING);
		}
    }
}
