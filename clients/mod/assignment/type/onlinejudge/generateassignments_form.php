...
class generateassignments_form extends moodleform {
    var $testcasecount=1;

    function generateassignments_form() {
        parent::moodleform();
    }
	...
	function definition() {
		...
		$repeatarray = array();
		//$mform->addElement('filemanager', 'templatefile', get_string('assignmenttemplate', 'assignment_onlinejudge'), null, array('subdirs' => 0, 'maxfiles' => 1, 'accepted_types' => array('*.Rnw')));
		$mform->addElement('filepicker', 'templatefile', get_string('assignmenttemplate', 'assignment_onlinejudge'), null,array('maxbytes' => $maxbytes, 'accepted_types' => '*'));
		$mform->addElement('text', 'testqtt', get_string('testqtt', 'assignment_onlinejudge'), $attributes='size="2"');
